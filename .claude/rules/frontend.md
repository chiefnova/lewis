...
- `apps/directory` is the public, anonymous patient directory served at `corridor.health`. It is anonymous-first and SEO-critical:
  ...
  - Do not import Clerk at the route level. Lazy-load Clerk only inside the connect-request flow and the eligibility-result handoff that needs to detect an existing `.corridor.health` session.
