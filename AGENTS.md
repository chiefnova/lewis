...
- IMPORTANT: never add `Co-Authored-By: Codex` trailers, `🤖 Generated with Codex` footers, or any AI attribution to commits or PRs. Write in first-person imperative as the user.
...
Treat AGENTS.md like code. Add an entry when Codex makes the same mistake twice, a code review catches something structural, or a correction is repeated. Delete entries Codex already follows unprompted. Keep it under ~200 lines — bloat degrades adherence. Large scoped rule sets go in `.Codex/rules/<topic>.md` with path-scoped frontmatter, not inline here.
